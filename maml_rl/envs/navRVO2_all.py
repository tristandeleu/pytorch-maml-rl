...
class NavRVO2Env_all(gym.Env):
    ...
    def __init__(self, task={}):
        ...
        radius = self._ped_radius # 0.05  is this the ped radius? shound't it be at least the safety distance
        maxSpeed = 0.2 
    ...
    def step(self, action):
        ...
        for i in range(self._num_ped):
            ...
            self._state[6+i*5: 6+i*5+4] = np.append(real_ped_state[i,:], [ai_velocity[0], ai_velocity[1]])
        ...
        done = ((np.abs(dx) < 0.1) and (np.abs(dy) < 0.1))
